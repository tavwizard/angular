...
class _ParseAST {
  ...
  parseAction:boolean;
  index:int;
  constructor(input:string, location:any, tokens:List<Token>, reflector:Reflector, parseAction:boolean) {
    this.input = input;
  ...
  parsePrimary(): AST {
    if (this.optionalCharacter($LPAREN)) {
      ...
      let elements = this.parseExpressionList($RBRACKET);
      this.expectCharacter($RBRACKET);
      ...
      let value = this.next.toNumber();
      this.advance();
      return new LiteralPrimitive(value);

    } else if (this.next.isString()) {
      let value = this.next.toString();
      this.advance();
      return new LiteralPrimitive(value);
  ...
  parseExpressionList(terminator:int):List<AST> {
    var result = [];
